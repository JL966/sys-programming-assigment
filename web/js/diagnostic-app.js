...
function fresh(){return {id:crypto.randomUUID(),started:new Date().toISOString(),rules:'2.3.0',firmware:'2.3.0',items:names.map((name,i)=>({id:i+1,name,status:'UNTESTED',reason:'尚未执行',advice:advice[i],history:[]}))};}
let run=fresh();
...
function render(){ $('filter').disabled=$('list').hidden;$('overall').textContent=summary(run.items);$('counts').textContent=['NORMAL','ABNORMAL','UNTESTED'].map((s,i)=>`${run.items.filter(x=>x.status===s).length} ${['正常','异常','未测试'][i]}`).join(' · ');$('list').innerHTML=run.items.filter(i=>!onlyBad||nonNormal(i)).map(i=>{const title=`<b>${String(i.id).padStart(2,'0')}　${e(i.name)}</b><span>${({NORMAL:'正常',ABNORMAL:'异常',UNTESTED:'未测试'})[i.status]}</span>`;return `<article class="row ${i.status} row-enter">${i.status==='NORMAL'?`<div class="row-title">${title}</div>`:`<details><summary>${title}</summary><div class="detail"><p>${i.status==='ABNORMAL'?'疑似异常：':'原因：'}${e(i.reason)}</p><p>建议：${e(i.advice)}</p>${i.evidence?.length?`<details><summary>查看证据</summary><pre>${e(JSON.stringify(i.evidence,null,2))}</pre></details>`:''}</div></details>`}</article>`;}).join('');}
...
async function execute(ids,newRun){if(busy)return;if(!board?.connected){$('notice').textContent='请先连接已烧录的被测板。';return;}if(newRun||run.rules!=='2.3.0')run=fresh();busy=true;stop=false;$('cancel').disabled=false;for(const id of ['connect','swap','all','single','history','resume'])$(id).disabled=true;$('task').hidden=false;
 try{for(const id of ids){if(stop)break;const item=run.items[id-1];run.checkpoint={id,remaining:ids.slice(ids.indexOf(id)),phase:'PREPARING'};await save(run);$('display-help').textContent=devices[id]?.display||'';showDevice(id);$('user-note').value='';$('title').textContent=item.name;$('count').textContent=`${id} / ${names.length}`;$('steps').innerHTML='<li>正在读取设备，请稍候。</li>';$('choices').innerHTML='';
...
$('toggle-list').onclick=()=>{const hidden=!$('list').hidden;$('list').hidden=hidden;$('filter').disabled=hidden;$('toggle-list').textContent=hidden?'展开清单':'收起清单';$('toggle-list').setAttribute('aria-expanded',String(!hidden));};$('filter').onclick=()=>{if($('list').hidden)return;onlyBad=!onlyBad;$('filter').textContent=onlyBad?'显示全部':'只显示异常和未测试';render();};$('html').onclick=()=>download(run,'html');$('json').onclick=()=>download(run,'json');
