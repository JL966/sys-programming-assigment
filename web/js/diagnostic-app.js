...
function render(){ $('overall').textContent=summary(run.items);$('counts').textContent=['NORMAL','ABNORMAL','UNTESTED'].map((s,i)=>`${run.items.filter(x=>x.status===s).length} ${['正常','异常','未测试'][i]}`).join(' · ');$('list').innerHTML=run.items.filter(i=>!onlyBad||nonNormal(i)).map(i=>{const title=`<b>${String(i.id).padStart(2,'0')}　${e(i.name)}</b><span>${({NORMAL:'正常',ABNORMAL:'异常',UNTESTED:'未测试'})[i.status]}</span>`;return `<article class="row ${i.status}">${i.status==='NORMAL'?`<div class="row-title">${title}</div>`:`<details><summary>${title}</summary><div class="detail"><p>${i.status==='ABNORMAL'?'疑似异常：':'原因：'}${e(i.reason)}</p><p>建议：${e(i.advice)}</p>${i.evidence?.length?`<details><summary>查看证据</summary><pre>${e(JSON.stringify(i.evidence,null,2))}</pre></details>`:''}</div></details>`}</article>`;}).join('');}
async function connect(which){if(busy&&which!=='aux')return;try{const b=new Board();await b.open();await b.ask(CMD.ROLE,0,0,0,[which==='board'?1:2]);if(which==='board'){await board?.close();board=b;if(!run.checkpoint)run=fresh();render();}else{await aux?.close();aux=b;if(runner)runner.aux=b;}$('ports').textContent=`被测板：${board?.connected?'已连接':'未连接'} · 辅助板：${aux?.connected?'已连接':'未连接'}`;$('swap').disabled=!(board?.connected&&aux?.connected);$('notice').textContent='连接成功：统一固件 v2.2.0；被测板待机01，辅助板02。';}catch(err){$('notice').textContent=err.message;}}
$('connect').onclick=()=>connect('board');$('aux').onclick=()=>connect('aux');$('swap').onclick=async()=>{if(!busy){[board,aux]=[aux,board];try{await board.ask(CMD.ROLE,0,0,0,[1]);await aux.ask(CMD.ROLE,0,0,0,[2]);}catch(e){$('notice').textContent='角色同步失败，请重新连接：'+e.message;return;}run=fresh();render();$('notice').textContent='已交换两板，建立新记录。';}};
...
$('filter').onclick=()=>{onlyBad=!onlyBad;$('filter').textContent=onlyBad?'显示全部':'只显示异常和未测试';render();};$('html').onclick=()=>download(run,'html');$('json').onclick=()=>download(run,'json');
...
async function reconnectExtension(wiring,r){
 ...
 $('ports').textContent=`被测板：已连接 · 辅助板：${aux?.connected?'已连接':'未连接'}`;
