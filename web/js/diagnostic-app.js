...
function choice(steps,buttons){const visibleSteps=steps.map(step=>step.replace('请选择无法确认','请选择异常'));$('steps').classList.toggle('plain-steps',visibleSteps.some(s=>s.startsWith('请操作：')));$('steps').innerHTML=visibleSteps.map(s=>`<li>${e(s)}</li>`).join('');$('choices').innerHTML='';return new Promise(resolve=>{resolveChoice=value=>{$('choices').innerHTML='';resolve(value);};for(const name of buttons.filter(value=>value!=='无法确认')){const b=document.createElement('button');b.textContent=name;if(name==='正常')b.className='manual-normal';if(name==='异常')b.className='manual-abnormal';b.onclick=()=>{resolveChoice=null;$('choices').innerHTML='';resolve(name);};$('choices').append(b);}});}
