...
import {durations as D,temperature} from './diagnostic-feedback.js';
const wait=ms=>new Promise(r=>setTimeout(r,ms));
export class Runner {
 constructor(board,aux,ui,backup,update=()=>{}){Object.assign(this,{board,aux,ui,backup,update});this.deadline=0;this.live='';this.cancelled=false;this.data=[];}
 ...
 check(){if(this.cancelled)throw Error('用户跳过或取消');if(!this.board.connected || (this.id>=15&&this.aux&&!this.aux.connected))throw Error('连接中断');}
 ...
 async run(id,attempt){this.id=id;this.attempt=attempt;this.data=[];this.cancelled=false;const ticker=setInterval(()=>this.report(),100);const heartbeat=setInterval(()=>{if(!this.cancelled&&id!==2){this.board.ask(CMD.HELLO).catch(()=>{});if(id>=15)this.aux?.ask(CMD.HELLO).catch(()=>{});}},1500);try{
  ...
  if(id>=15&&!this.aux?.connected){await this.prompt(['现在可点击上方“连接辅助板”，选择第二个USB串口。','连接完成后点击开始；没有辅助板请跳过。']);if(!this.aux?.connected)return {status:'UNTESTED',reason:'未连接辅助板；连接第二块同固件学习板后复测。'};}
 ...
 }catch(e){return {status:this.cancelled||!this.board.connected||(id>=15&&!this.aux?.connected)||e.message.includes('连接中断')?'UNTESTED':'ABNORMAL',reason:e.message,evidence:this.data};}
 finally{clearInterval(ticker);clearInterval(heartbeat);this.clock();await this.board.ask(CMD.STOP).catch(()=>{});if(id===13)await wait(1100);await this.board.ask(CMD.ROLE,0,0,0,[1]).catch(()=>{});if(id>=15){await this.aux?.ask(CMD.STOP).catch(()=>{});await this.aux?.ask(CMD.ROLE,0,0,0,[2]).catch(()=>{});}}}
}
