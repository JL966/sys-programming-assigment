...
try {
 ...
 $log=@()
 foreach($name in 'main','diagnostic','hardware') {
 ...
 $lines=& "$Keil/C51/BIN/BL51.exe" 'output/main.obj,output/diagnostic.obj,output/hardware.obj,source/STCBSP_V3.6.LIB' 'TO' 'output/AcceptanceDiagnostic' 'XDATA(0x0000-0x06FF)' 'PRINT(list/AcceptanceDiagnostic.m51)' 2>&1
 $log+=$lines;$lines | Write-Output
